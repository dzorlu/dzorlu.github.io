...
Let's distill how to run a Mixture-of-Experts (MoE) model on 8 GPUs using a 2-D device mesh with Expert Parallel (EP) and FSDP2 (fully_shard). 
This short post explains who stores which weights, where all-to-all happens, and includes minimal code (meshes, wrapping, verification).
...
[![MoE 3D Visualization](https://tinkerings.dev/static/moe-preview.png){width=50px}](https://tinkerings.dev/static/moe.html)
*Click for interactive 3D visualization*
