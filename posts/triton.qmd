...
The most fundamental truth about GPU programming: memory is a flat 1D array. Your `tensor[M, N]` doesn't exist—it's just multi-dimensional indexing over `M*N` consecutive bytes.
...
We launch `(M // BLOCK_M) * (N // BLOCK_N)` kernels. Each kernel computes one tile of C with shape `[BLOCK_M, BLOCK_N]` by:
- Loading blocks from one row-strip of A
- Loading blocks from one column-strip of B
